...
#2. - findBraggs
def findBraggs(A, obj=None, rspace=True, min_dist=5, thres=0.25, gwidth=5, maskon=True, sig=5, show=False, angle=0, update_obj=True):
    '''
    ...
    Input:
        ...
        maskon      - Optional : Boolean, if True then a Gaussian mask (width=len(A)/sig) will be used to
                                    remove low q high intensity signals before finding Bragg peaks.
        sig         - Optional : Float for the width of the central mask (width = len(A)/sig)
