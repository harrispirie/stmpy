...
class ScrollableMap:
    ...
    def __init__(self, volume, ax, ch_names=[], **kwargs):
        ...
        if volumetype(volume) == 'a 3D array':
            ...
            if ch_names:
                if len(volume) == len(ch_names):
                   self.ax.ch_names = ch_names
                    ...
                    print('ERROR: number of ch_names does not match number of channels in volume!')
            else:
                if len(volume) == 1:
                    self.ax.ch_names = ['Topo']
                else:
                    print('ERROR: please provide %d channel names!'%len(volume))
            ...
            print('ERROR: volume argument only supports input types of a 2D/3D array or a list of 2D arrays!')
